...
The `cli-tool` crate demonstrates building command-line interfaces using the `clap` crate with derive macros.

### Tool Overview

A simple CLI application that greets users with customizable subcommands.

### Features

- **Subcommands**: `hello` and `goodbye` commands
- **Named arguments**: Uses short (`-n`) and long (`--name`) flags
- **Clap derive macros**: Modern declarative CLI parsing

### Available Commands

| Command | Description | Usage |
|---------|-------------|-------|
| `hello` | Greet someone with hello | `cargo run -- hello --name <NAME>` |
| `goodbye` | Say goodbye to someone | `cargo run -- goodbye --name <NAME>` |
...
# Using short flag
cargo run -- hello -n Bob
# Output: Hello, Bob!

# Say goodbye
cargo run -- goodbye --name Charlie
